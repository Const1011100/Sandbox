...
// ================================================================================================
// ================================================================================================
//*-Task №696 Simple string reversal II (7kyu)
/*
У цьому ката вам буде задано рядок та два індекси (a та b).
Ваше завдання — перевернути частину цього рядка між цими двома індексами включно.

str = "codewars", a = 1, b = 5 --> "cawedors"
str = "cODEWArs", a = 1, b = 5 --> "cAWEDOrs"
Вводити лише малі та великі літери.

Перший індекс a завжди буде меншим за довжину рядка;
другий індекс b може бути більшим за довжину рядка.
*/
...
function solve(st, a, b) {
  const newArr = st
    ...
    .slice(a, b + 1)
    .reverse();
  const finalArr = st.split('');
  finalArr.splice(a, b - a + 1, ...newArr);
  return finalArr.join('');
}

// Цікаві варіанти з Codewars
const solve_ = (string, a, b) =>
  string.slice(0, a) +
  string
    .slice(a, b + 1)
    .split('')
...
//*-Task №697 (7kyu)
/*
Дано два слова та літеру. Поверніть одне слово, яке є комбінацією обох слів, об'єднаних у місці,
де задана літера вперше з'являється в кожному слові. Повернуте слово повинно мати початок першого
слова та кінець другого, з роздільною літерою посередині. Можна припустити, що обидва слова
міститимуть роздільну літеру.
("hello", "world", "l")       ==>  "held"
...
function stringMerge(string1, string2, letter) {
  ...
  let mergedString = string1.slice(0, index1) + string2.slice(index2);
...
// ================================================================================================
// ================================================================================================
//*-Task №698 Band name generator (7kyu)
/*
Моя подруга хоче нову назву для свого гурту. Їй подобаються гурти, які використовують формулу:
"The" + іменник з першою літерою з великої літери, наприклад:

"dolphin" -> "The Dolphin"

Однак, коли іменник ПОЧИНАЄТЬСЯ та ЗАКІНЧУЄТЬСЯ однією й тією ж літерою, вона любить повторювати
іменник двічі та з'єднувати їх разом першою та останньою літерами, об'єднаними в одне слово
(БЕЗ "The" попереду), ось так:

"alaska" -> "Alaskalaska"
Напишіть функцію, яка приймає іменник як рядок та повертає її бажану назву гурту, записану як рядок.
*/
...
function bandNameGenerator(str) {
  ...
  if (first === last) {
    // Повторити слово, друге без першої літери
    const combined = str + str.slice(1);
    return combined[0].toUpperCase() + combined.slice(1);
  } else {
    // Просто "The" + слово з великої
    return 'The ' + str[0].toUpperCase() + str.slice(1);
  }
}
// Цікаві варіанти з Codewars
const bandNameGenerator_ = (s) =>
  s[0] != s[s.length - 1]
    ? 'The ' + s[0].toUpperCase() + s.slice(1)
    : s[0].toUpperCase() + s.slice(1) + s.slice(1);
// ================================================================================================
// ================================================================================================
//*-Task №699 Last Survivor (7kyu)
/*
Ваша робота в E-Corp одночасно нудна та складна. Її не полегшує той факт, що всі постійно хочуть
зустрітися з вами, а кімнати для переговорів завжди зайняті!

У цьому ката вам буде надано масив. Кожне значення представляє кімнату для переговорів.
Ваше завдання? Знайти першу порожню кімнату та повернути її індекс
(Примітка: у деяких тестових випадках може бути більше однієї порожньої кімнати).

'X' --> busy
'O' --> empty
Якщо всі кімнати зайняті, поверніть "Немає доступних!"
*/
...
// ================================================================================================
// ================================================================================================
//*-Task №700 Going to the cinema (7kyu)
/*
Мій друг Джон любить ходити в кіно. Він може вибрати між системою А та системою Б.

Система A: він купує квиток (15 доларів) щоразу
Система B: він купує картку (500 доларів) та перший квиток за 0,90 від вартості квитка,
потім за кожен наступний квиток він платить 0,90 від вартості попереднього квитка.

Приклад:
Якщо Джон йде в кіно 3 рази:

Система A: 15 * 3 = 45
Система B: 500 + 15 * 0,90 + (15 * 0,90) * 0,90 + (15 * 0,90 * 0,90) * 0,90
( = 536,5849999999999, без округлення для кожного квитка)

Джон хоче знати, скільки разів йому потрібно піти в кіно, щоб кінцевий результат Системи B,
округлений до наступного долара, був дешевшим, ніж Система A.

Функція movie має 3 параметри:
card (ціна картки),
ticket (звичайна ціна квитка),
perc (частина від того, що він заплатив за попередній квиток) та повертає перше n таке, що
ceil(ціна Системи B) < ціна Системи A.

Більше прикладів:
movie(500, 15, 0.9) має повернути 43
(з карткою загальна ціна 634, з квитками 645)
movie(100, 10, 0.95) має повернути 24
(з карткою загальна ціна 235, з квитками 240)
*/
...
// Цікаві варіанти з Codewars
function movie_(card, ticket, perc) {
  var costA = (n = 0),
    costB = card;
