...
// ================================================================================================
// ================================================================================================
//*-Task №317 Localize The Barycenter of a Triangle (8kyu)
/*
Медіани трикутника - це відрізки, які об'єднують вершини з серединами їх протилежних сторін.
Три медіани трикутника перетинаються в одній точці, яка називається барицентром або центроїдом.
Дано трикутник, визначений декартовими координатами його вершин, нам потрібно локалізувати його барицентр або центроїд.

Функція barTriang отримує координати трьох вершин A, B і C як три різні аргументи та виводить
координати барицентру O в масиві [xO, yO]

Ось як має працювати наша задана функція: результат координат має бути виражений до чотирьох
десяткових знаків (округлений результат).
Ви знаєте, що координати барицентра задаються такими формулами.
...
function barTriang(A, B, C) {
    ...
    const roundedXO = Math.round(xO * 10000) / 10000;
    const roundedYO = Math.round(yO * 10000) / 10000;

    return [roundedXO, roundedYO];
}
// Цікаві варіанти з Codewars
function barTriang_(p1, p2, p3) {
...
// ================================================================================================
// ================================================================================================
//*-Task №318 Quadratic Coefficients Solver (8kyu)
/*
У цьому ката ви повинні знайти коефіцієнти квадратного рівняння даних двох коренів (x1 і x2).
Рівняння матиме вигляд ax^2 + bx + c = 0

Тип повернення – це вектор, що містить коефіцієнти рівнянь у порядку (a, b, c).
Оскільки розв’язків цієї задачі нескінченно багато, фіксуємо a = 1.
Пам’ятайте, що корені можна записати так (x-x1) * (x-x2) = 0
*/
...
// Цікаві варіанти з Codewars
function quadratic_(x1, x2) {
    return [1, -(x2 + x1), (x1 * x2)]
}
// ================================================================================================
// ================================================================================================
//*-Task №319 Classy Extentions (8kyu)
/*
Класні розширення, ця ката головним чином спрямована на нове оновлення JS ES6, яке представляє ключове
слово extends. У вас буде попередньо завантажено клас Animal, тому вам слід редагувати лише клас Cat.

Ваше завдання полягає в тому, щоб виконати клас Cat, який розширює Animal, і замінити метод speak на
повернення імені котів + нявкання. напр. 'Mr Whiskers meows.'.
Атрибут name передається з this.name (JS).
*/
...
// ================================================================================================
// ================================================================================================
//*-Task №320 Job Matching #1 (8kyu)
/*
Давайте створимо систему підбору партнерів, яка допоможе знаходити вакансії для розробників на основі низки факторів.
Одним із найпростіших, але найважливіших факторів є компенсація. Як розробники ми знаємо, скільки
грошей нам потрібно, щоб підтримувати наш спосіб життя, тому ми, як правило, маємо приблизне уявлення
про мінімальну зарплату, якою ми були б задоволені.

Давайте спробуємо. Ми створимо функцію match (job_matching у Python), яка візьме кандидата та вакансію,
яка поверне логічне значення, яке вказує, чи вакансія відповідає кандидату.

Кандидат матиме мінімальну зарплату, тому це виглядатиме так:
нехай кандидат = {
   minзарплата: 120000
}
Робота матиме максимальну зарплату, тому це виглядатиме так:
let job = {
   Максимальна зарплата: 140000
}
Якщо мінімальна заробітна плата кандидата або максимальна заробітна плата не вказано, видайте помилку.
Для дійсного збігу мінімальна зарплата кандидата має бути меншою або дорівнювати максимальній зарплаті на посаді.
Однак давайте також включимо 10% вільного місця (вираховується з мінімальної зарплати кандидата), якщо кандидат є
рок-зіркою, яка любить програмувати на Codewars у вільний час. Компанія, яка пропонує роботу, може щось виробити.
*/
