...
//*-Task №816 (7kyu)
/*
Ви отримуєте нову роботу в Eggman Movers. Ваше перше завдання — написати метод, який дозволить
адміністративному персоналу ввести ім'я особи та повернути її роль у компанії.

Вам буде надано масив об'єктних літералів, що містять поточних співробітників компанії.
Ваш код повинен знайти співробітника з відповідними firstName та lastName, а потім повернути роль
для цього співробітника, або, якщо співробітник не знайдено, він має повернути "Does not work here!".

Масив попередньо завантажено, і на нього можна посилатися за допомогою змінної employees.
Він використовує таку структуру:

let employees = [ {firstName: "Dipper", lastName: "Pines", role: "Boss"}, ...... ]
У масиві немає дублікатів імен, а передане ім'я буде одним рядком з пробілом між ім'ям та прізвищем,
тобто Jane Doe або просто ім'я.
*/
...
function findEmployeesRole(name) {
  ...
  // Якщо прізвище не передано — імені недостатньо для пошуку
  if (!lastName) return 'Does not work here!';
...
// Цікаві варіанти з Codewars
function findEmployeesRole_(name) {
